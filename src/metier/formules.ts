...
export const INDICES_ESSENCE: Record<string, IndiceEssence> = {
...
type FormuleMeta = {
  ...
  uniteEntree?: Record<string, string>;
  uniteSortie?: string;
  hypotheses?: string;
  domaine?: string;
};

function withMeta<F extends (...args: never[]) => unknown>(
  fn: F,
  meta: FormuleMeta
): F & { meta: FormuleMeta } {
 ...
 */
export const calculerPci = withMeta(
  (humPct: number): number => {
 ...
 * Source : ITEBE 2004.
 */
export const calculerPoidsAjuste = withMeta(
  (volumeM3: number, densiteKgM3: number, humiditePct: number): number => {
    const h = Math.min(95, Math.max(0, humiditePct));
 ...
 */
export const calculerVolApparent = withMeta(
  (nbTas: number, longueurM: number, largeurM: number, hauteurM: number): number =>
 ...
 * Source : ITEBE 2004.
 */
export const calculerVolReel = withMeta(
  (volApparentM3: number, foisonnement: number): number =>
 ...
 * Source : ITEBE 2004.
 */
export const calculerMasse = withMeta(
  (volReelM3: number, densiteKgM3: number): number =>
 ...
 */
export const calculerEnergie = withMeta(
  (masseTonnes: number, pciMWhParT: number): number =>
 ...
 */
export const estimerHumiditeDepuisFoisonnement = withMeta(
  (foisonnement: number): number =>
 ...
 */
export const estimerVolumeParHa = withMeta(
  (popParHa: number, diametreCm: number, surfaceHa: number, densiteKgM3: number, hauteurMoy: number): number => {
    const nbTiges = popParHa || 0;
    const dM = (diametreCm || 0) / 100;
    const volUnit = (Math.PI / 4) * dM * dM * (hauteurMoy || 20);
    const volTotal = volUnit * nbTiges * (surfaceHa || 0);
 ...
 * Source : ITEBE 2004.
 */
export const indicesPonderes = withMeta(
  (essences: Array<{ id: string; pct: number }> | null | undefined): IndiceEssence => {
    if (!essences?.length) return INDICES_ESSENCE['melange']!;
    const total = essences.reduce((s, e) => s + e.pct, 0) || 100;
    const get = (id: string): IndiceEssence =>
      INDICES_ESSENCE[id] || INDICES_ESSENCE['melange']!;
    return {
